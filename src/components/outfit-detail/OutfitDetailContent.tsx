...
import type { Outfit, Advisor } from '@/types'
...
export function OutfitDetailContent({ outfit }: OutfitDetailContentProps) {
  ...
  const shareToSocial = (platform: string) => {
    ...
    switch (platform) {
      case 'whatsapp': {
        const txt = `¡Mira este increíble outfit "${outfit.nombre}" en Xianna! ${outfit.descripcion} ${link} #moda #estilo #xianna #outfit`
        navigator.clipboard.writeText(txt)
        toast.success('¡Texto copiado!', { description: 'Pégalo en WhatsApp para compartir este outfit.', duration: 4000 })
        setShowShareMenu(false)
        return
      }
      case 'facebook': {
        const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(link)}`
        window.open(url, '_blank'); setShowShareMenu(false); return
      }
      case 'instagram': {
        const txt = `${outfit.nombre} - ${outfit.descripcion} ${link} #moda #estilo #xianna #outfit`
        navigator.clipboard.writeText(txt)
        toast.success('¡Texto copiado!', { description: 'Pégalo en tu historia de Instagram.', duration: 4000 })
        setShowShareMenu(false)
        return
      }
      default: return
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 lg:gap-12 max-w-7xl mx-auto">
      ...
      <div className="space-y-4 order-1">
                ...
                {showShareMenu && (
                  <div className="absolute right-0 top-14 bg-white rounded-xl shadow-xl border border-gray-200 p-1 z-50 min-w-[160px]">
                    <div className="space-y-0.5">
                      <button onClick={() => shareToSocial('whatsapp')} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-gray-50 rounded-lg transition-colors">
                        <WhatsAppIcon sx={{ color: '#25D366', fontSize: 20 }} />
                        <span className="text-sm text-gray-700">WhatsApp</span>
                      </button>
                      <button onClick={() => shareToSocial('facebook')} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-gray-50 rounded-lg transition-colors">
                        <FacebookIcon sx={{ color: '#1877F2', fontSize: 20 }} />
                        <span className="text-sm text-gray-700">Facebook</span>
                      </button>
                      <button onClick={() => shareToSocial('instagram')} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-gray-50 rounded-lg transition-colors">
        ...
        <div className="flex items-center justify-center gap-6 text-sm text-gray-500">
          <div className="flex items-center gap-2">
            <Bookmark className="w-4 h-4 text-[#E61F93]" />
            <span>{outfit.favoritos} me gusta</span>
          </div>
        </div>
      </div>

      {/* PRNDAS - SOLO MÓVIL (debajo de outfit) */}
      ...
      {/* Contenido (der en xl) */}
      <div className="space-y-8 order-3 xl:order-2">
        {/* Header */}
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <Badge variant="secondary" className="bg-[#E61F93]/10 text-[#E61F93] border-[#E61F93]/20 font-medium">
              <Sparkles className="w-3 h-3 mr-1" />
              {outfit.estilo}
            </Badge>
            {outfit.ocasiones?.map((ocasion, index) => (
              <Badge key={index} variant="outline" className="border-[#FDE12D] bg-[#FDE12D]/10 text-gray-700">
                {ocasion}
              </Badge>
            )) || (
              <Badge variant="outline" className="border-[#FDE12D] bg-[#FDE12D]/10 text-gray-700">
                Casual
              </Badge>
            )}
          </div>
          ...
          <div className="space-y-1">
            <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 leading-tight">{outfit.nombre}</h1>
            ...
            {(advisor || advisorLoading) && (
              <div className="flex items-start gap-2 text-sm text-gray-600">
                ...
                {advisorLoading ? (
                  ...
                  <button onClick={() => setShowAdvisorDialog(true)} className="text-[#E61F93] hover:text-[#E61F93]/80 font-medium transition-colors underline">
                    {advisor.nombre}
                  </button>
                ) : null}
              </div>
        ...
        {/* Descripción */}
        <div className="space-y-4">
          <p className="text-gray-600 leading-relaxed text-lg">{outfit.descripcion}</p>
        </div>
        ...
        {outfit.estilos?.descripcion && (
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-r from-[#E61F93]/5 to-[#FDE12D]/5 rounded-2xl p-6 border border-gray-100">
              <p className="text-gray-700 leading-relaxed">{outfit.estilos.descripcion}</p>
            </div>
          </div>
        )}

        {/* PRNDAS - SOLO DESKTOP (se queda donde estaba) */}
        {outfit.prendas && outfit.prendas.length > 0 && (
        ...
        <div className="space-y-4 pt-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <Button
              ...
              className={`flex-1 h-12 text-base font-semibold rounded-xl transition-all duration-300 ${
                isFavorite ? 'bg-[#E61F93] hover:bg-[#E61F93]/90 text-white'
          ...
          <div className="bg-gradient-to-r from-[#E61F93]/5 to-[#FDE12D]/5 border border-[#E61F93]/20 rounded-2xl p-6 text-center">
            ...
            <Button onClick={() => router.push('/formulario')} className="bg-[#E61F93] hover:bg-[#E61F93]/90 text-white font-semibold rounded-xl px-6">
              Descubrir mi estilo
            </Button>
          </div>
          ...
          {!user && (
            <div className="bg-[#FDE12D]/10 border border-[#FDE12D]/30 rounded-2xl p-6 text-center">
              ...
              <Button onClick={() => router.push('/auth/register')} className="bg-[#FDE12D] hover:bg-[#FDE12D]/90 text-gray-900 font-semibold rounded-xl">
                Crear cuenta
              </Button>
            </div>
          )}
      ...
      <Dialog open={showAdvisorDialog} onOpenChange={setShowAdvisorDialog}>
        <DialogContent className="max-w-md mx-4 max-h-[90vh] overflow-y-auto">
          ...
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold text-gray-900">Asesor de moda</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              {advisor.biografia && (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed">{advisor.biografia}</p>
                </div>
              )}

              <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100">
                {advisor.correo && (
                  <a href={`mailto:${advisor.correo}`} className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                    <Mail className="w-4 h-4" />
                    Contactar por email
                  </a>
                )}
                {advisor.portfolio_url && (
                  <a href={advisor.portfolio_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-[#E61F93]/10 text-[#E61F93] hover:bg-[#E61F93]/20 rounded-lg transition-colors">
                    <ExternalLink className="w-4 h-4" />
                    Ver portfolio
                  </a>
                )}
                {advisor.contact_link && advisor.contact_link !== advisor.portfolio_url && (
                  <a href={advisor.contact_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-[#FDE12D]/20 text-gray-700 hover:bg-[#FDE12D]/30 rounded-lg transition-colors">
                    <ExternalLink className="w-4 h-4" />
                    Más información
                  </a>
                )}
